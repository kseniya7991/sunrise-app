...
export default function NextMorning({ currentDate, data }) {
  ...
  function setInfo(currentDate, data) {
    ...
    setWeather(
      `${nextMorningWeather[0].main + " " + nextMorningWeather[0].description}`
    );
    ...
    if (timeNext) {
      let h = timeNext.getHours();
      let m = timeNext.getMinutes();
      ...
      setTimeToNextSun(
        `${nextSunH}:${nextSunM.length === 1 ? "0" + nextSunH : nextSunM}`
      );
    }
  }
